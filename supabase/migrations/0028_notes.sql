-- Generic, reusable note timeline. A `notes` row is a timestamped, authored entry attached to any
-- entity via (entity_type, entity_id) — leads today; clients/cases/consultations reuse it later by
-- extending the entity_type check + RLS. Each entry can be ADDRESSED (resolved_at — collapses in the
-- UI), HIDDEN (hidden_at — kept for history, off by default), edited (edited_at), or hard-deleted by
-- its author or an admin. Replaces the single `leads.notes` text field.
--
...
create policy "notes_update" on public.notes
  ...
  using (firm_id = public.current_firm_id() and public.authorize('leads.edit'))
  with check (firm_id = public.current_firm_id() and public.authorize('leads.edit'));
