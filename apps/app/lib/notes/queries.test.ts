...
describe("mapNoteRow", () => {
  ...
  test("falls back to Unknown for a null or unmapped author", () => {
    expect(mapNoteRow(row({ created_by_id: null }), names).authorName).toBe("Unknown")
    expect(mapNoteRow(row({ created_by_id: "ghost" }), names).authorName).toBe("Unknown")
  })

  test("carries the lifecycle timestamps through", () => {
    const v = mapNoteRow(row({ resolved_at: "2026-06-21T00:00:00Z", edited_at: "2026-06-20T16:00:00Z" }), names)
...
describe("partitionNotes", () => {
  ...
  test("hides hidden notes by default but always counts them", () => {
    const { visible, hiddenCount } = partitionNotes([note("a", false), note("b", true)], { showHidden: false })
    expect(visible.map((n) => n.id)).toEqual(["a"])
    expect(hiddenCount).toBe(1)
  })

  test("includes hidden notes when showHidden is on", () => {
    const { visible, hiddenCount } = partitionNotes([note("a", false), note("b", true)], { showHidden: true })
